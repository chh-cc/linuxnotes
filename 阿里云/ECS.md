...
- 升级ECS实例对我的云上业务的配置有什么影响吗？
  ...
  升级按量付费实例时，需要预先停止实例。升级包年包月实例时，**需要重启实例**后新配置才能生效。升级操作会使您的业务短暂中断，建议您在业务不繁忙时进行升级。升级后的实例能无缝对接业务，您无需重新配置云服务器ECS环境。
...
- 企业级实例和共享型实例的区别？
  ...
  企业级实例采用**固定CPU调度模式**。每个vCPU绑定到一个物理CPU超线程，实例间无CPU资源争抢，实例计算性能稳定且有严格的SLA保证。
