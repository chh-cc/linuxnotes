...
VPC每个客户都是独立的网络环境，客户可以自定义网络的IP网段、网络架构等。
...
- 负载均衡类
  ...
  多台机器同时对外提供服务，比如slb+多台tomcat，通过slb将内网ecs上的8080端口间接暴露给公网请求。

- 公网IP类

  为ecs直接绑定公网主要的场景（少见，出于安全考虑）：

  ecs需要暴露给公网（可采用SLB）

  需要公网远程登录到这台服务器（可以用slb四层转发ssh的请求到一台ecs，然后将这台ecs与后端的ecs的ssh key打通

  ecs上部署的服务需要去公网调用及请求第三方的服务及接口（可采用NAT网关的SNAT功能）

### 带宽

阿里云流量带宽峰值、流量带宽费用针对的是出口 ，入口流量峰值是不受限制的，所以DDOS攻击不会产生额外的流量费用，因为都是入口流量。

若使用5Mbps峰值带宽，每天业务产生的流量超过5.25GB，这时使用固定带宽才划算，否则优先使用按量付费。

## 云端存储

存储的数据结构类型决定了我们选择什么样类型的存储产品：

- 结构化数据：常见如关系型数据库中的数据表的数据
- 半结构化数据：具有可识别的模式并且可以解析的文本数据文件，如xml数据文件、json数据文件
- 非结构化数据：比如文本文档、图片、视频、日志文件、代码文件等

云端存储产品可以分为两大类：

- 数据库类云产品，用于结构化数据及半结构化数据的持久化存储
- 块存储（云盘）、共享块存储（共享云盘）、共享文件存储、OSS对象存储用于非结构化数据的持久化存储

## 云端缓存

前端缓存的应用就是静态缓存，如CDN

后端缓存的应用分为动态请求（asp、php、jsp）缓存和数据库热点数据缓存
...
4. 实例配置
   ...
   共享型：采用随机的更贪婪的调度CPU的模式，一个核的使用可能回调度分配给不同的云主机实例。

   独享型：固定调度CPU，一个核的使用回固定调度给某云主机实例
...
对象存储是以对象的形式管理数据的。对象和文件最大的不同就是在文件的基础上增加了元数据。一般对象分为3个部分：数据、元数据、对象ID。

对象的数据通常是无结构的数据；对象的元数据指的是对象的相关描述，比如图片的大小、文档的拥有者等；对象ID则是一个全局的唯一标识符，用来区分对象的。

对象存储（分布式文件系统）可以通过REST-ful API接口对对象进行访问操作，并且可以调用HTTP动词（GET、PUT、DELETE等）描述操作。
...
七层负载均衡，在中大型项目中不适合作为业务请求的流量入口，特别是电商高并发的场景。如果是中小型应用，用户请求的QPS远远达不到50000，其实还是优先选七层的SLB作为流量入口。

对于用户请求流量入口，一般采用二层（LVS DR）/四层或硬件负载来实现，从而实现抗并发及分流，然后将请求转发给后端七层。

相比于四层SLB，七层SLB可直接将证书配置在其中，不用去后端所在的ECS的nginx中配置证书。
...
如果ECS有公网IP和私网IP，禁用公网网卡就会影响负载均衡服务，因为在有公网网卡的情况下，默认路由会走公网，如禁用就无法回包。如果一定要禁用就要修改默认路由为私网才不会影响。但需要考虑业务是否对公网有依赖，如通过公网访问RDS等。
