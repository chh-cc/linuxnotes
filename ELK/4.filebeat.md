...
#采集同一台服务器上的多个服务，使用 - 进行分隔
- input_type: log
  #日志文件路径列表，可用通配符，不递归
  paths:
  - /var/log/*.log
  #定义标签，如rs项目debug类型的日志，标签可以设为：rs_debug
  tags: ["rs_debug"]
...
同一条日志的数据可能分多行进行打印，那么在使用ELK收集日志的时候就需要将属于同一条日志的多行数据进行合并。

方法：使用 multiline 多行合并插件，如果有filebeat，就无需在logstash中配置
