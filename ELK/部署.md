...
```shell
# tar zxvf /usr/local/src/elasticsearch-6.5.4.tar.gz
-C /usr/local/
# echo '
...
http.port: 9200
#discovery.zen.ping.unicast.hosts: ["192.168.1.191",
"192.168.1.192"]
...
network.host 绑定节点IP。
http.port rest api端⼝。
discovery.zen.ping.unicast.hosts 提供其他
Elasticsearch 服务节点的单点⼴播发现功能。
...
注意：
1.确保堆内存最⼩值（Xms）与最⼤值（Xmx）的⼤⼩相同，防⽌
程序在运⾏时改变堆内存⼤⼩。
...
kafka {
 ...
 bootstrap_servers => "192.168.1.190:9092,
192.168.1.191:9092, 192.168.1.192:9092"
 }
}
