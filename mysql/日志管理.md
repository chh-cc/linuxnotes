...
作用：记录启动、关闭、日常运行的状态信息、警告、错误，当数据库出现任何故障，建议首先查看此日志
...
```css
（1）statement（5.6默认）SBR(statement based replication) ：基于sql语句的日志记录，记录对数据进行修改的sql
（2）ROW(5.7 默认值) RBR(ROW based replication) ：基于行的日志记录，记录每一行的数据变化(用户看不懂，需要工具分析)
（3）mixed（混合）MBR(mixed based replication)模式  ：以上两种模式的混合，默认采用statement，在某些特殊情况下会自动切换为row
...
由于日志是以**二进制的方式存储**的，不能直接读取，需要通过二进制日志查询工具mysqlbinlog来查看，具体语法：
...
如果mysql每次更新操作都要写进硬盘，磁盘也要找到对应的记录再更新，整个过程IO成本、查询成本很高，所以mysql用了WAL技术，具体就是，**当有一条记录需要更新的时候，把数据页加载到内存，直接在内存里更新，然后会把“在某个数据页上做了什么修改”记录到redo log中。**
