...
![img](https://gitee.com/c_honghui/picture/raw/master/img/20210701105348.webp)

当我们去查询数据时候会先 拿着我们当前查询的页去缓冲池中查询当前页是否在缓冲池中，如果在则直接获取。

update时，则会直接修改 Buffer中的值。这个时候，缓冲池中的数据就和磁盘中存储的数据不一致了，称为脏页。每隔一段时间，Innodb存储引擎就会把脏页数据刷入磁盘。
...
![img](https://gitee.com/c_honghui/picture/raw/master/img/20210615104922.webp)
...
InnoDB(默认的存储引擎)的表：
-rw-r----- 1 mysql mysql    8636 Apr 18 11:37 time_zone.frm #列相关信息
...
在表结构设计中,除了将列定义为字符外,还需要定义字符对应的字符集,因为每种字符在不同的字符集下对应不同的二进制值,通常推荐默认设置为utf8

而随着移动互联网飞速发展,推荐把默认字符设置为utf8mb4,否则，某些 emoji 表情字符无法在 UTF8 字符集下存储
...
```mysql
ALTER TABLE emoji_test CHARSET utf8mb4; #只对新增列有效，旧列无效
ALTER TABLE emoji_test CONVERT TO CHARSET utf8mb4; #对所有列生效
```
...
表的类型不同，会对应mysql不同的存取机制，存取机制又称为存储引擎。
